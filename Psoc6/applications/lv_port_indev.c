...
//        struct rt_touch_data touch_data[1];  // 只需要读取第一个触摸点
//
//        /* 延迟初始化触摸设备 */
...
//        }
        ...
        /* 读取触摸数据 */
//        if (rt_device_read(touch_dev, 0, touch_data, 1) > 0)
//        {
//            /* 判断触摸状态 */
//            if (touch_data[0].event == RT_TOUCH_EVENT_DOWN ||
//                touch_data[0].event == RT_TOUCH_EVENT_MOVE)
//            {
//                last_x = touch_data[0].x_coordinate;
//                last_y = touch_data[0].y_coordinate;
//                data->state = LV_INDEV_STATE_PR;
//            }
//            else
//            {
...
static bool touchpad_is_pressed(void)
{
    /*Your code comes here*/
    uint8_t status = 0;
    status = GT911_IsPressed(); // 读取触摸屏状态
    if (status == 1) {
        return true;
    }
    else {
...
static void touchpad_get_xy(lv_coord_t * x, lv_coord_t * y)
{
    ...
    (*x) = GT911_GetX(); // 获取触摸屏坐标X
    (*y) = GT911_GetY(); // 获取触摸屏坐标Y 
}
