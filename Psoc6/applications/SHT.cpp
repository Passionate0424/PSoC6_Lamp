...
// static char disp_AHT_buffer[15];
...
//     float t = (sht31.readTemperature() + Air_Data[0]) / 2 - 13; // 读取温度，平均与BMP温度
...
//     // lvgl 显示数据
//     rt_snprintf(disp_AHT_buffer, sizeof(disp_AHT_buffer), "%.2f°C", T_H_Data[0]);
...
//         lv_label_set_text(guider_ui.monitor_label_temperature_now, disp_AHT_buffer);
...
//     }
//     rt_snprintf(disp_AHT_buffer, sizeof(disp_AHT_buffer), "%.2f%%", T_H_Data[1]);
...
//     {
//         rt_mutex_take(lv_mutex, RT_WAITING_FOREVER); // 确保线程安全
//         lv_label_set_text(guider_ui.monitor_label_humidity_now, disp_AHT_buffer);
//         if (SHT_index == 60)
//         {
